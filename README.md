<div align="center">

# Matheus

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=32&duration=2800&pause=2000&color=00FF41&center=true&vCenter=true&width=600&lines=Back-End+Developer;Automation+Specialist;Game+Developer;AI+Integration+Expert" alt="Typing SVG" />

<br/>

### Junior Back-End Developer @ CRRT - UEMG | Information Systems Student

*Building efficient systems through automation and intelligent integrations*

<br/>

[![Game Portfolio](https://img.shields.io/badge/Game_Portfolio-z4z.studio-FF6B6B?style=for-the-badge)](https://z4z.studio)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/matheus-nascimento-b97609279/)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:matheusnmto@gmail.com)

</div>

---

## About Me

```typescript
const matheus = {
    location: "Passos, Minas Gerais, Brazil",
    role: "Junior Back-End Developer",
    workplace: "CRRT - State University of Minas Gerais (UEMG)",
    education: "Information Systems @ UEMG (Penultimate Year)",
    specialization: ["Process Automation", "AI Integrations", "Workflow Optimization"],
    focus: ["Sustainability", "E-waste Management", "Game Development"],
    currentProjects: ["FaceFut", "Restorant", "FreePago", "Farah"]
};
```

---

## Professional Experience

### Junior Back-End Developer | CRRT - UEMG
**Centro de Recondicionamento de Computadores | State University of Minas Gerais**

- Developing backend solutions using Node.js, C#, Java, and Golang
- Implementing automation workflows with n8n for organizational processes
- Integrating AI capabilities into existing systems
- Managing MySQL databases and optimizing queries
- Organizing e-waste collection campaigns and sustainability initiatives
- Contributing to computer reconditioning programs for digital inclusion

**Key Achievements:**
- Successfully automated multiple internal workflows, reducing manual processing time
- Implemented AI-powered solutions for process optimization
- Led sustainable technology initiatives within the university

---

## Tech Stack

### Backend & Languages
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)
![C#](https://img.shields.io/badge/C%23-239120?style=for-the-badge&logo=csharp&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![Go](https://img.shields.io/badge/Go-00ADD8?style=for-the-badge&logo=go&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white)

### Frontend
![Next.js](https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=nextdotjs&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![TailwindCSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white)

### Automation & AI
![n8n](https://img.shields.io/badge/n8n-EA4B71?style=for-the-badge&logo=n8n&logoColor=white)
![AI Integration](https://img.shields.io/badge/AI_Integration-412991?style=for-the-badge&logo=openai&logoColor=white)
![Workflow Automation](https://img.shields.io/badge/Workflow_Automation-FF6B6B?style=for-the-badge&logo=zapier&logoColor=white)
![Process Optimization](https://img.shields.io/badge/Process_Optimization-00C853?style=for-the-badge&logo=processwire&logoColor=white)

### Database & Tools
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)

### Game Development
![Unity](https://img.shields.io/badge/Unity-000000?style=for-the-badge&logo=unity&logoColor=white)
![Game Design](https://img.shields.io/badge/Game_Design-FF0080?style=for-the-badge&logo=gameandwatch&logoColor=white)

---

## Featured Projects

<div align="center">

### Core Automation & AI Projects

<table>
<tr>
<td width="50%">

#### Process Automation with n8n
**Workflow Optimization & Integration**

Specialized in building complex automation workflows using n8n, connecting multiple systems and APIs to streamline organizational processes. Focus on reducing manual tasks and improving operational efficiency through intelligent automation.

**Key Technologies:**
- n8n workflow automation
- API integrations
- Webhook implementations
- Database connections
- AI-powered workflows

`n8n` `Automation` `Integration` `AI`

</td>
<td width="50%">

#### FreePago
**Payment Solution Platform**

Complete payment solution project featuring comprehensive branding, logo design, and landing page development. Modern fintech interface with focus on user experience and conversion optimization.

**Features:**
- Custom logo and branding
- Responsive landing page
- Modern UI/UX design
- Payment flow optimization

`Next.js` `Design` `Fintech` `Payment`

</td>
</tr>
</table>

---

### Game Development

<table>
<tr>
<td width="50%" align="center">

![Roll Your Life!](https://img.shields.io/badge/Roll_Your_Life!-Hackathon_Project-FF6B6B?style=for-the-badge)

**Roll Your Life!**

Hackathon team project featuring innovative game mechanics and creative design. Developed collaboratively under time constraints with focus on unique gameplay experience.

[Play Now](https://z4z.studio)

`Unity` `Game Jam` `Team Project`

</td>
<td width="50%" align="center">

![Hey Brother!](https://img.shields.io/badge/Hey_Brother!-Hackathon_Project-4ECDC4?style=for-the-badge)

**Hey Brother!**

Narrative-focused game developed during game jam. Collaborative project emphasizing storytelling and player engagement through interactive gameplay.

[Play Now](https://z4z.studio)

`Unity` `Game Jam` `Narrative`

</td>
</tr>
</table>

**Complete Game Portfolio:** [z4z.studio](https://z4z.studio)

</div>

---

## Sustainability & Social Impact

### E-waste Management & Digital Inclusion

As part of my role at CRRT (Centro de Recondicionamento de Computadores) at UEMG, I actively contribute to sustainability initiatives focused on electronic waste management and digital inclusion:

**Key Initiatives:**
- Computer reconditioning and refurbishment programs
- E-waste collection campaigns and proper disposal education
- Environmental awareness programs for responsible technology use
- Digital inclusion through reconditioned technology distribution
- Sustainable technology practices within academic environments

These initiatives combine technical expertise with environmental responsibility, demonstrating that technology development and sustainability can work together to create positive social impact.

---

<div align="center">

### Let's Connect

Open to collaborating on projects involving:
**Automation** | **AI Integration** | **Backend Development** | **Game Development** | **Sustainability**

Feel free to explore the repositories and reach out for collaboration opportunities.

<img src="https://raw.githubusercontent.com/Trilokia/Trilokia/379277808c61ef204768a61bbc5d25bc7798ccf1/bottom_header.svg" />

</div>
